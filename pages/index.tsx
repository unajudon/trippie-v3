...
export default function Home() {
  ...
  return (
    <main className="relative min-h-screen w-full bg-gradient-to-br from-[#7041F2] to-[#10DBAC] flex items-center justify-center px-4 py-10 font-poppins text-white overflow-hidden">
      ...
      {/* Floating card with intro content */}
      <motion.div
      ...
      >
        <motion.h1
          className="text-3xl md:text-4xl font-bold mb-4 leading-snug flex items-center gap-2"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.3 }}
        >
          <SparkleIcon className="w-6 h-6 text-white animate-pulse" />
          Welcome to Trippie Quiz
        </motion.h1>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className="w-full bg-white/20 text-white font-medium px-6 py-3 rounded-xl shadow-md hover:bg-white hover:text-[#7041F2] transition-all duration-200"
        >
          Start the Quiz
        </motion.button>
      </motion.div>

      {/* Optional floating Trippie card */}
      <motion.img
        src="/card-float.png"
        alt="Trippie Card"
